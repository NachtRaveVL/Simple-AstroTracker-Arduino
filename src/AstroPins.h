...
inline pintype_t pinNumberForPinChannel(int8_t pinChannel) { return isValidChannel(pinChannel) && pinChannel >= 0 ? (pintype_t)(apin_virtual + pinChannel) : apin_none; }
// Returns expanded pin channel # [0,16) to use for a given virtual pin # (100+), else pin # [0,99] if not virtual (<100), else -1/invalid
inline uint8_t pinChannelOrPinNumber(pintype_t pinNumber) { return isValidPin(pinNumber) ? (pinNumber >= apin_virtual ? (pinNumber - apin_virtual) % 16 : pinNumber) : (uint8_t)-1; }
// Returns expander position # [0,Max] to use for a given virtual pin # (100+), else -1/invalid if not virtual (<100)
inline aposi_t expanderPosForPinNumber(pintype_t pinNumber) { return isValidPin(pinNumber) && pinNumber >= apin_virtual ? (pinNumber - apin_virtual) / 16 : (aposi_t)-1; }
// Returns expander position # [0,Max] to use for a given expanded pin channel # [0,16*Max), else -1/invalid if not expanded
inline aposi_t expanderPosForPinChannel(int8_t pinChannel) { return isValidChannel(pinChannel) && pinChannel >= 0 ? pinChannel / 16 : (aposi_t)-1; }
...
inline int8_t pinChannelForMuxerChannel(uint8_t muxChannel) { return muxChannel != (uint8_t)-1 && muxChannel != (uint8_t)apinchnl_none ? -(int8_t)(1 + constrain(muxChannel,0,125)) : apinchnl_none; }
// Returns expanded pin channel # [+0,16*Max) to use for a given expander channel # [0,16*Max), else -127/none
inline int8_t pinChannelForExpanderChannel(uint8_t expChannel) { return expChannel != (uint8_t)-1 && expChannel != (uint8_t)apinchnl_none ? (int8_t)constrain(expChannel,0,127) : apinchnl_none; }
...
struct AstroPin {
    ...
    Astro_PinMode mode;                                     // Pin mode setting
    ...
    inline bool isValid() const { return isValidPin(pin) && mode != Astro_PinMode_Undefined; }
    inline bool isVirtual() const { return isValidPin(pin) && pin >= apin_virtual; }
    ...
    inline bool isInput() const { return mode == Astro_PinMode_Digital_Input ||
                                         mode == Astro_PinMode_Digital_Input_PullUp ||
                                         mode == Astro_PinMode_Digital_Input_PullDown ||
                                         mode == Astro_PinMode_Analog_Input; }
    inline bool canRead() const { return isValid() && isInput(); }
    inline bool isOutput() const { return mode == Astro_PinMode_Digital_Output ||
                                          mode == Astro_PinMode_Digital_Output_PushPull ||
                                          mode == Astro_PinMode_Analog_Output; }
    inline bool canWrite() const { return isValid() && isOutput(); }
    inline bool isDigital() const { return mode == Astro_PinMode_Digital_Input ||
                                           mode == Astro_PinMode_Digital_Input_PullUp ||
                                           mode == Astro_PinMode_Digital_Input_PullDown ||
                                           mode == Astro_PinMode_Digital_Output ||
                                           mode == Astro_PinMode_Digital_Output_PushPull; }
    inline bool isAnalog() const { return mode == Astro_PinMode_Analog_Input ||
                                          mode == Astro_PinMode_Analog_Output; }
...
struct AstroPinData : public AstroSubData
{
    pintype_t pin;                                          // Pin number
    Astro_PinMode mode;                                     // Pin mode
...
public:
    ...
    inline const AstroDigitalPin &getInterruptPin() const { return _interrupt; }
    inline uint8_t getChannelBits() const { return _channelBits; }
    inline uint8_t getSelectedChannel() const { return _channelSelect; }

protected:
    AstroPin _signal;                                       // Muxed signal pin (unused channel #)
    AstroDigitalPin _chipEnable;                            // Muxing chip enable pin (optional)
    AstroDigitalPin _interrupt;                             // Muxing interrupt pin (optional)
...
// Note: Virtual pins should always have a pin # >= apin_virtual (100), and are separated
//       into groups of 16 per indexed expander (e.g. pins # 100-115 = expander[0], pins #
//       116-131 = expander[1], etc.).
class AstroPinExpander {
public:
    AstroPinExpander();
    AstroPinExpander(aposi_t expanderPos, uint8_t channelBits, IoAbstractionRef ioRef,
                     AstroDigitalPin interruptPin = AstroDigitalPin());
    ...
    inline aposi_t getExpanderPos() const { return _expander; }
    inline uint8_t getChannelBits() const { return _channelBits; }
    inline IoAbstractionRef getIoAbstraction() { return _ioRef; }
    inline const AstroDigitalPin &getInterruptPin() const { return _interrupt; }

protected:
    const aposi_t _expander;                                // Expander #/index (for virtual pin #)
    uint8_t _channelBits;                                   // Channel select bits (# of bus pins available)
    IoAbstractionRef _ioRef;                                // IoAbstraction instance
    AstroDigitalPin _interrupt;                             // Expander interrupt pin (optional)
    bool _usingISR;                                         // Using ISR flag
};

#endif // /ifdef ASTRO_USE_MULTITASKING

#endif // /ifndef AstroPin_H
