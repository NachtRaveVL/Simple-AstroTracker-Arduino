...
#undef ASTRO_UI_ENABLE_DEBUG_MENU
#endif
#endif // /ifdef NDEBUG

#include <Astruino.h>
#ifdef ASTRO_USE_GUI
#ifndef AstroBaseUI_H
#define AstroBaseUI_H

#include "AstroUIDefines.h"
#include "AstroUIInlines.hh"
#include "AstroUIData.h"
#include "AstroUIStrings.h"
...
#include "AstroDisplayDrivers.h"
#include "AstroInputDrivers.h"
#include "AstroRemoteControls.h"
#include "AstroMenus.h"
#include "AstroOverviews.h"
...
class AstruinoBaseUI : public AstroUIInterface, public CustomDrawing {
public:
    // Base UI constructor.
    AstruinoBaseUI(String deviceUUID,                                     // Device UUID hex string for remote controllability
    ...
    virtual AstroUIData *init(AstroUIData *uiData = nullptr) override;      // UIData instance
    ...
    void init(uint8_t updatesPerSec,                                        // Updates per second (1 to 10)
              Astro_DisplayTheme displayTheme,                              // Display theme to apply
    ...
    inline int getVRAMBufferRows() const { return _isBufferedVRAM && (_display && _display->getGraphicsRenderer()) ? ASTRO_UI_SPRITE_MAXYSIZE : 0; }
    ...
    // Using tcUnicode fonts over GFXfont (Adafruit) fonts
    inline bool isTcUnicodeFonts() const { return _isTcUnicodeFonts; }
    // Using buffered VRAM for non-flickering sprite animations
    inline bool isBufferedVRAM() const { return _isBufferedVRAM; }

    // UIData accessor
    inline AstroUIData *getUIData() { return _uiData; }
    // Input driver accessor
    inline AstroInputDriver *getInput() { return _input; }
    // Display driver accessor
    inline AstroDisplayDriver *getDisplay() { return _display; }
    // Remote server accessor
    inline TcMenuRemoteServer *getRemoteServer() { return _remoteServer; }
    // Overview screen accessor
    inline AstroOverview *getOverview() { return _overview; }
    // Home menu accessor
    inline AstroHomeMenu *getHomeMenu() { return _homeMenu; }
...
protected:
    ...
    AstroUIData *_uiData;                                   // Astro UI data (strong)
    AstroInputDriver *_input;                               // Input driver (owned)
    AstroDisplayDriver *_display;                           // Display driver (owned)
    TcMenuRemoteServer *_remoteServer;                      // Remote control server (owned)
    Vector<AstroRemoteControl *, ASTRO_UI_REMOTECONTROLS_MAXSIZE> _remotes; // Remote controls list (owned)
    AstroPin *_backlight;                                   // Backlight control (owned)
    time_t _blTimeout;                                      // Backlight timeout (UTC)
    AstroOverview *_overview;                               // Overview screen (owned)
    AstroHomeMenu *_homeMenu;                               // Home menu screen (owned)
...
#include "AstroDisplayDrivers.hpp"
#include "AstroOverviews.hpp"

#endif // /ifndef AstroBaseUI_H
#endif
