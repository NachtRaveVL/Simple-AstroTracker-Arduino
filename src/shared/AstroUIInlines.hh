/*  Astruino: Simple automation controller for DIY astronomical tracking systems.
    ...
    Astruino UI Common Inlines
*/

#include <Astruino.h>
#ifdef ASTRO_USE_GUI
#ifndef AstroUIInlines_HPP
#define AstroUIInlines_HPP
...
inline AstruinoBaseUI *getBaseUI() { return reinterpret_cast<AstruinoBaseUI *>(getUI()); }

// Returns the first theme in parameter list that isn't undefined, allowing defaulting chains to be nicely defined.
inline Astro_DisplayTheme definedThemeElse(Astro_DisplayTheme theme1, Astro_DisplayTheme theme2) {
    return theme1 != Astro_DisplayTheme_Undefined ? theme1 : theme2;
}


// LCD Display Setup
struct LCDDisplaySetup {
    Astro_BacklightMode ledMode;        // LCD backlight pin mode (default: Astro_BacklightMode_Normal)
    bool isDFRobotShield;               // Using DF robot shield

    inline LCDDisplaySetup(Astro_BacklightMode ledModeIn = Astro_BacklightMode_Normal, bool isDFRobotShieldIn = false) : ledMode(ledModeIn), isDFRobotShield(isDFRobotShieldIn) { ; }

    static inline LCDDisplaySetup usingDFRobotShield() { return LCDDisplaySetup(Astro_BacklightMode_Normal, true); }
};

// Standard Pixel Display Setup (U8g2OLED & AdaGfx/AdaTFT)
struct PixelDisplaySetup {
    Astro_DisplayRotation rotation;     // Display orientation/rotation (default: R0)
    ...
    Astro_BacklightMode ledMode;        // Backlight/LED/BL pin mode (default: Astro_BacklightMode_Normal)
...
#endif
    Astro_ST77XXKind st77Kind;          // ST7735 tag color or ST7789 screen resolution (default: undef/-1), if ST77XX

    inline PixelDisplaySetup(Astro_DisplayRotation rotationIn = Astro_DisplayRotation_R0, pintype_t dcPinIn = -1, pintype_t resetPinIn = -1, pintype_t ledPinIn = -1, Astro_BacklightMode ledModeIn = Astro_BacklightMode_Normal, uint8_t ledBitResIn = DAC_RESOLUTION,
#ifdef ESP32
                             uint8_t ledChannelIn = 1,
#endif
#ifdef ESP_PLATFORM
                             float ledFrequencyIn = 1000,
#endif
                             Astro_ST77XXKind st77KindIn = Astro_ST77XXKind_Undefined)
...
struct TFTDisplaySetup {
    ...
    float ledFrequency;                 // Backlight PWM output frequency, if PWM/ESP
#endif
    Astro_ST77XXKind st77Kind;          // ST7735 tag color (default: undef/-1), if ST7735 (ST7789 enums not used)

    inline TFTDisplaySetup(Astro_DisplayRotation rotationIn = Astro_DisplayRotation_R0, pintype_t ledPinIn = -1, Astro_BacklightMode ledModeIn = Astro_BacklightMode_Normal, uint8_t ledBitResIn = DAC_RESOLUTION,
#ifdef ESP32
                           uint8_t ledChannelIn = 1,
#endif
#ifdef ESP_PLATFORM
...
struct UIDisplaySetup {
    ...
    inline Astro_DisplayRotation getDisplayRotation() const { return dispCfgType == Pixel ? dispCfgAs.gfx.rotation : dispCfgType == TFT ? dispCfgAs.tft.rotation : Astro_DisplayRotation_R0; }
    inline pintype_t getBacklightPin() const { return dispCfgType == LCD ? (pintype_t)(dispCfgAs.lcd.isDFRobotShield ? 10 : 3) : dispCfgType == Pixel ? dispCfgAs.gfx.ledPin : dispCfgType == TFT ? dispCfgAs.tft.ledPin : apin_none; }
    inline Astro_BacklightMode getBacklightMode() const { return dispCfgType == LCD ? dispCfgAs.lcd.ledMode : dispCfgType == Pixel ? dispCfgAs.gfx.ledMode : dispCfgType == TFT ? dispCfgAs.tft.ledMode : Astro_BacklightMode_Normal; }
...
struct RotaryControlSetup {
    Astro_EncoderSpeed encoderSpeed;    // Encoder cycling speed (detent freq)

    inline RotaryControlSetup(Astro_EncoderSpeed encoderSpeedIn = Astro_EncoderSpeed_HalfCycle) : encoderSpeed(encoderSpeedIn) { ; }
};
...
struct ButtonsControlSetup {
    ...
    inline ButtonsControlSetup(uint8_t repeatSpeedIn = ASTRO_UI_KEYREPEAT_SPEED, bool isDFRobotShieldIn = false) : repeatSpeed(repeatSpeedIn), isDFRobotShield(isDFRobotShieldIn) { ; }

    static inline ButtonsControlSetup usingDFRobotShield() { return ButtonsControlSetup(ASTRO_UI_KEYREPEAT_SPEED, true); }
};
...
struct ESP32TouchControlSetup {
    ...
    Astro_ESP32Touch_HighRef highVoltage;       // High reference voltage (default: V_2V7)
    Astro_ESP32Touch_LowRef lowVoltage;         // Low reference voltage (default: V_0V5)
    Astro_ESP32Touch_HighRefAtten attenuation;  // High reference voltage attention (default: V_1V)

    inline ESP32TouchControlSetup(uint8_t repeatSpeedIn = ASTRO_UI_KEYREPEAT_SPEED, uint16_t switchThresholdIn = 800, Astro_ESP32Touch_HighRef highVoltageIn = Astro_ESP32Touch_HighRef_V_2V7, Astro_ESP32Touch_LowRef lowVoltageIn = Astro_ESP32Touch_LowRef_V_0V5, Astro_ESP32Touch_HighRefAtten attenuationIn = Astro_ESP32Touch_HighRefAtten_V_1V) : repeatSpeed(repeatSpeedIn), switchThreshold(switchThresholdIn), highVoltage(highVoltageIn), lowVoltage(lowVoltageIn), attenuation(attenuationIn) { ; }
};
...
struct MatrixControlSetup {
    ...
    Astro_EncoderSpeed encoderSpeed;    // Encoder cycling speed (optional)

    inline MatrixControlSetup(millis_t repeatDelayIn = 850, millis_t repeatIntervalIn = 350, Astro_EncoderSpeed encoderSpeedIn = Astro_EncoderSpeed_HalfCycle) : repeatDelay(repeatDelayIn), repeatInterval(repeatIntervalIn), encoderSpeed(encoderSpeedIn) { ; }
};

// Touchscreen Input Setup
struct TouchscreenSetup {
    Astro_TouchscreenOrientation orient; // Touchscreen orientation tuning (default: Same)
    #ifdef ASTRO_UI_ENABLE_XPT2046TS
        SPIClass *spiClass;
    #endif

    #ifndef ASTRO_UI_ENABLE_XPT2046TS
        inline TouchscreenSetup(Astro_TouchscreenOrientation orientIn = Astro_TouchscreenOrientation_Same) : orient(orientIn) { ; }
    #else
        inline TouchscreenSetup(Astro_TouchscreenOrientation orientIn = Astro_TouchscreenOrientation_Same, SPIClass *spiClassIn = ASTRO_USE_SPI) : orient(orientIn), spiClass(spiClassIn) { ; }
    #endif
};
...
#endif // /ifndef AstroUIInlines_HPP
#endif
