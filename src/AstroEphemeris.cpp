...
static inline double rad(double value) { return value * M_PI / 180.0; }
static inline double deg(double value) { return value * 180.0 / M_PI; }
...
{
    double m = rad(astroNormalizeDegrees(meanAnomalyDegrees));
...
static void orbitalXYZ(const AstroOrbitalElements &elements, double *xOut, double *yOut, double *zOut, double *radiusOut = nullptr, double *longitudeOut = nullptr)
{
    ...
    double n = rad(elements.node);
    double i = rad(elements.inclination);
    double wv = v + rad(elements.perihelion);
...
static AstroOrbitalElements planetElements(Astro_TargetId targetId, double d)
{
    switch (targetId) {
        case Astro_Target_Mercury: return {48.3313 + 3.24587e-5*d, 7.0047 + 5.00e-8*d, 29.1241 + 1.01444e-5*d, 0.387098, 0.205635 + 5.59e-10*d, 168.6562 + 4.0923344368*d};
        case Astro_Target_Venus:   return {76.6799 + 2.46590e-5*d, 3.3946 + 2.75e-8*d, 54.8910 + 1.38374e-5*d, 0.723330, 0.006773 - 1.302e-9*d, 48.0052 + 1.6021302244*d};
        case Astro_Target_Mars:    return {49.5574 + 2.11081e-5*d, 1.8497 - 1.78e-8*d, 286.5016 + 2.92961e-5*d, 1.523688, 0.093405 + 2.516e-9*d, 18.6021 + 0.5240207766*d};
        case Astro_Target_Jupiter: return {100.4542 + 2.76854e-5*d, 1.3030 - 1.557e-7*d, 273.8777 + 1.64505e-5*d, 5.20256, 0.048498 + 4.469e-9*d, 19.8950 + 0.0830853001*d};
        case Astro_Target_Saturn:  return {113.6634 + 2.38980e-5*d, 2.4886 - 1.081e-7*d, 339.3939 + 2.97661e-5*d, 9.55475, 0.055546 - 9.499e-9*d, 316.9670 + 0.0334442282*d};
        case Astro_Target_Uranus:  return {74.0005 + 1.3978e-5*d, 0.7733 + 1.9e-8*d, 96.6612 + 3.0565e-5*d, 19.18171 - 1.55e-8*d, 0.047318 + 7.45e-9*d, 142.5905 + 0.011725806*d};
        case Astro_Target_Neptune: return {131.7806 + 3.0173e-5*d, 1.7700 - 2.55e-7*d, 272.8461 - 6.027e-6*d, 30.05826 + 3.313e-8*d, 0.008606 + 2.15e-9*d, 260.2471 + 0.005995147*d};
...
{
    double obliquity = rad(23.4393 - 3.563e-7 * d);
    double xe = x;
    ...
    return AstroEquatorialCoordinates(astroNormalizeDegrees(deg(ra)) / 15.0, deg(dec));
}
...
static AstroEquatorialCoordinates resolveSun(double d)
{
    ...
    double lon = v + rad(sun.perihelion);
...
static AstroEquatorialCoordinates resolveMoon(double d)
{
    ...
    double mm = rad(astroNormalizeDegrees(moon.anomaly));
    double ms = rad(astroNormalizeDegrees(sun.anomaly));
    double dd = rad(astroNormalizeDegrees(lMoon - lSun));
    double ff = rad(astroNormalizeDegrees(lMoon - moon.node));

    double lonDeg = deg(lon);
    ...
    double latDeg = deg(atan2(z, sqrt(x*x + y*y)));
...
bool astroResolveSolarSystemTarget(Astro_TargetId targetId, int64_t unixTime, AstroEquatorialCoordinates *coordinatesOut)
{
    if (!coordinatesOut) { return false; }
    double d = daysFromJ2000(unixTime);
    if (targetId == Astro_Target_Sun) {
        *coordinatesOut = resolveSun(d);
        return true;
    }
    if (targetId == Astro_Target_Moon) {
        *coordinatesOut = resolveMoon(d);
        return true;
    }
    if (targetId < Astro_Target_Mercury || targetId > Astro_Target_Neptune) { return false; }
    ...
    double xp, yp, zp;
    orbitalXYZ(planetElements(targetId, d), &xp, &yp, &zp);
